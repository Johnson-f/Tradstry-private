...
pub async fn calculate_options_performance_metrics(
    conn: &Connection,
    time_condition: &str,
    ...
    let winners_hold_time =
        calculate_options_winners_hold_time(conn, time_condition, time_params).await?;
    let losers_hold_time =
        calculate_options_losers_hold_time(conn, time_condition, time_params).await?;

    // Calculate advanced metrics for options
    let (trade_expectancy, edge, payoff_ratio) =
        calculate_expectancy_and_edge_options(conn, time_condition, time_params).await?;
    let kelly_criterion =
        calculate_kelly_criterion_options(conn, time_condition, time_params).await?;
    ...
    let consistency_ratio =
        calculate_consistency_ratio_options(conn, time_condition, time_params).await?;
    let (monthly_win_rate, quarterly_win_rate) =
        calculate_periodic_win_rates_options(conn, time_condition, time_params).await?;
    let system_quality_number =
        calculate_system_quality_number_options(conn, time_condition, time_params).await?;
