...
pub async fn transform_brokerage_transactions(
    ...
    info!(
        "Automatic transformation is disabled. Please use the manual merge feature at /api/brokerage/transactions/merge"
    );
    Ok(())
}
...
#[allow(dead_code)]
pub async fn transaction_already_transformed(
    conn: &Connection,
    ...
    let mut rows = stmt
        .query(libsql::params![snaptrade_transaction_id])
        .await
            ...
            && let Ok(trans_data) = serde_json::from_str::<Value>(&raw)
        {
                ...
                (trans_symbol, trans_price, trans_units, trans_trade_date)
            {
                if is_option {
                    ...
                    let mut check_rows = check_stmt
                        .query(libsql::params![sym, date, price, units, trans_institution])
                        .await?;
...
#[allow(dead_code)]
pub fn parse_trade_date(date_str: &str) -> Result<chrono::DateTime<Utc>> {
...
#[allow(dead_code)]
pub async fn transform_to_stock(
    conn: &Connection,
    transaction: &Value,
    ...
    let brokerage_name = transaction.get("institution").and_then(|v| v.as_str());
    ...
    let _stock_id: i64 = if let Some(row) = rows.next().await? {
        row.get(0)?
    } else {
        return Err(anyhow::anyhow!("Failed to get inserted stock ID"));
    };
...
#[allow(dead_code)]
pub async fn transform_to_option(
    conn: &Connection,
    transaction: &Value,
    _snaptrade_transaction_id: &str,
    _user_id: &str,
    ...
    let _option_symbol = transaction.get("option_symbol").and_then(|v| v.as_str());
    ...
    let _option_id: i64 = if let Some(row) = rows.next().await? {
        row.get(0)?
    } else {
...
pub async fn migrate_add_brokerage_name_column(conn: &Connection) -> Result<()> {
    ...
    // Check and add to stocks table
    {
        let check_col = conn
            .prepare(
                "SELECT COUNT(*) FROM pragma_table_info('stocks') WHERE name = 'brokerage_name'",
            )
            .await?;
        ...
        if let Some(row) = rows.next().await? {
            ...
            if count == 0 {
                conn.execute(
                    "ALTER TABLE stocks ADD COLUMN brokerage_name TEXT",
                    libsql::params![],
                )
                .await
    ...
    // Check and add to options table
    {
        let check_col = conn
            .prepare(
                "SELECT COUNT(*) FROM pragma_table_info('options') WHERE name = 'brokerage_name'",
            )
            .await?;
        let mut rows = check_col.query(libsql::params![]).await?;
        if let Some(row) = rows.next().await? {
            ...
            if count == 0 {
                conn.execute(
                    "ALTER TABLE options ADD COLUMN brokerage_name TEXT",
                    libsql::params![],
                )
                .await
